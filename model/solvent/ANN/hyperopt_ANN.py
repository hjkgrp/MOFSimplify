...
trials, best_params = optimize(X_train, y_train, [regression_target],
                               regression=False, hyperopt_step=steps,
                               arch=False, epochs=2000,
                               X_val=X_val, y_val=y_val,
                               model=False, path=path)      

trials_filename = "trials_"+str(regression_target)
with open(trials_filename, 'wb') as f:
    pickle.dump(trials, f)

best_file = "best_"+str(regression_target) 
