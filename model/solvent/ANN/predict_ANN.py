# This file was the template for solvent_ANN.py. solvent_ANN.py is used by mofSimplify.
...
def normalize_data(df_train, df_test, fnames, lname, unit_trans=1, debug=False): # assumes gets Pandas dataframes with MOfs as rows and features as columns
...
df_train_all = pd.read_csv(path+"/train.csv").append(pd.read_csv(path+"/val.csv")) # TODO ask about this, use ANN/dropped_connectivity_dupes/train.csv ?
...
train_pred = np.round(model.predict(X_train))  # for prediction, X_train is just one row with the desired MOF
